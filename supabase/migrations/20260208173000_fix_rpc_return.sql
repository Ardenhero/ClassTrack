-- ============================================
-- FIX: Repair Students Page Crash
-- ============================================
...
RETURNS TABLE (
    ...
    sin TEXT,
    year_level TEXT,
    fingerprint_id TEXT, -- Legacy field, mapped from rfid_uid or fallback
    image_url TEXT
)
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
    ...
    SELECT DISTINCT 
        ...
        COALESCE(s.rfid_uid, s.fingerprint_id, 'N/A') as fingerprint_id, -- Handle both column names
        s.image_url
    FROM students s
