...
export function ProfileProvider({ children }: { children: ReactNode }) {
        ...
        const initializeProfile = async () => {
            ...
            if (stored) {
                try {
                    ...
                    // Background hydration for flags/photo if missing from cache
                    supabase.from('instructors')
                        .select('is_super_admin, image_url, assigned_room_ids')
                        .eq('id', parsed.id)
                        .maybeSingle()
                        .then(({ data }: { data: { is_super_admin: boolean, image_url: string | null, assigned_room_ids: string[] | null } | null }) => {
                            if (data) {
                                const updated = {
                                    ...parsed,
                                    is_super_admin: data.is_super_admin,
                                    image_url: data.image_url,
                                    assigned_room_ids: data.assigned_room_ids
                                };
                                setProfile(updated);
            ...
            if (cookieProfileId) {
                ...
                let query = supabase
                    .from('instructors')
                    .select('id, name, department_id, role, pin_enabled, is_super_admin, image_url, assigned_room_ids');
    ...
    const refreshProfile = async () => {
        ...
        const profileIdToQuery = profile?.id || user.id;
        const uuidRegex = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i;

        // Prevent crash: only query if it's a valid UUID
        if (!uuidRegex.test(profileIdToQuery)) {
            console.warn("[refreshProfile] Skipping refresh for legacy or invalid ID:", profileIdToQuery);
            return;
        }
        ...
        const { data: instructorData, error } = await supabase
            .from('instructors')
            .select('id, name, department_id, role, pin_enabled, is_super_admin, image_url, assigned_room_ids')
            .eq('id', profileIdToQuery)
            .maybeSingle();

        if (!error && instructorData) {
            const updatedProfile: Profile = {
                id: instructorData.id,
                name: instructorData.name || "Department Admin",
                role: (instructorData.role as "admin" | "instructor") || "instructor",
                department_id: instructorData.department_id,
                has_pin: instructorData.pin_enabled,
                is_super_admin: instructorData.is_super_admin || false,
                image_url: instructorData.image_url,
                assigned_room_ids: instructorData.assigned_room_ids
            };
