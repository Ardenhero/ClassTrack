...
export async function POST(request: NextRequest) {
    try {
        ...
        const { data: actor } = await supabase
            .from("instructors")
            .select("id, role, is_super_admin, department_id")
            .eq("id", profileId)
            .single();
        ...
        const { data: target } = await supabase
            .from("instructors")
            .select("id, auth_user_id, name, department_id")
        ...
        // Scope check: non-super-admins can only reset PINs within their department
        if (!actor.is_super_admin && actor.department_id !== target.department_id) {
