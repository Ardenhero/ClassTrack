...
export async function POST(request: Request) {
    ...
    try {
        ...
        let pendingCommand = existing.pending_command || null;
        if (pendingCommand) {
            await supabase
                .from('kiosk_devices')
        ...
        const { data: classes } = await supabase
            .from('classes')
            .select('id, name, start_time, end_time, year_level, instructor_id, day_of_week, instructors!classes_instructor_id_fkey(name, role)')
            .eq('room_id', roomId)
        ...
        const todayClasses = (classes || [])
            ...
            .map((c: any) => {
                ...
                const instructorObj = Array.isArray(c.instructors) ? c.instructors[0] : c.instructors;

                // v3.5: Strictly ensure ONLY true instructors are shown, not admins
                const isActualInstructor = instructorObj?.role === 'instructor';
                const instructorName = isActualInstructor ? (instructorObj?.name || 'Unknown') : 'Unknown Instructor';
        ...
        if (classIds.length > 0) {
            const { data: enrollments } = await supabase
                .from('enrollments')
                .select('class_id, student_id, students(id, name, year_level, fingerprint_slot_id, fingerprint_locked)')
            ...
            students = (enrollments || [])
                ...
                .map((e: any) => ({
                    ...
                    fingerprint_slot_id: e.students.fingerprint_slot_id,
                    fingerprint_locked: !!e.students.fingerprint_locked,
                    class_id: e.class_id,
                }));
        }
