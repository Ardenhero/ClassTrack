...
export async function POST(request: NextRequest) {
    try {
        ...
        if (mode === "auto") {
            ...
            const classIdsAffected = affectedClasses.map(c => c.id);
            const { data: enrollments } = await supabase
                .from("enrollments")
                .select("student_id, class_id")
                .in("class_id", classIdsAffected);

            if (enrollments && enrollments.length > 0) {
                // Deduplicate by student_id — each student gets ONE notification
                const uniqueStudentIds = Array.from(new Set(enrollments.map(e => e.student_id)));
                
                // Delete any existing suspension notifications for this date to prevent duplicates
                await supabase
                    .from("notifications")
                    .delete()
                    .eq("type", "no_class")
                    .eq("title", `System Suspension: ${date}`)
                    .in("student_id", uniqueStudentIds);

                const notifications = uniqueStudentIds.map(studentId => ({
                    student_id: studentId,
                    type: "no_class",
                    title: `System Suspension: ${date}`,
                    message: finalNote || `All classes have been suspended for ${date}. ${typeLabel}.`,
                }));
                ...
                if (notifyError) {
                    console.error("[DeclareSuspensions] Notification error:", notifyError);
                }
            }
