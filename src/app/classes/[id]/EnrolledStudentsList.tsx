...
export default function EnrolledStudentsList({
    ...
    return (
        <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 print:shadow-none print:border-0">
            ...
            {/* Student rows */}
            <div className="divide-y divide-gray-100 dark:divide-gray-700 pb-4">
                {filtered.map((student) => {
                    const badge = getAttendanceBadge(student.attendanceRate);
                    return (
                        <div key={student.enrollmentId} className="p-4 flex flex-col sm:flex-row sm:justify-between sm:items-center gap-3 print:py-2">
                            {/* Student info + attendance % */}
                            <div className="flex items-center min-w-0">
                                <div className="h-10 w-10 shrink-0 rounded-full bg-gray-100 dark:bg-gray-600 flex items-center justify-center text-gray-500 dark:text-gray-300 font-bold mr-3 print:h-8 print:w-8 overflow-hidden relative border border-gray-100 dark:border-gray-700">
                                ...
                                <div className="min-w-0 mr-3">
                                    <Link href={`/students/${student.studentId}`} className="block w-fit group">
                                        <p className="font-medium text-gray-900 dark:text-white truncate text-sm group-hover:text-nwu-red transition-colors">{student.studentName}</p>
                                    </Link>
