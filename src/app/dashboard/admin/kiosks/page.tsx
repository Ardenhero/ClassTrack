...
import {
    ...
    DoorClosed, Tag, Trash2, Loader2, Users, Save, Check, Eye, EyeOff
} from "lucide-react";
import {
...
import { DeviceInstructorSelector } from "../devices/DeviceInstructorSelector";

interface KioskDevice {
    device_serial: string;
    status: string | null;
    assigned_admin_ids: string[] | null;
    room_id: string | null;
    label: string | null;
    is_online: boolean;
...
export default function KioskInventoryPage() {
    ...
    const loadData = useCallback(async () => {
        ...
        try {
            ...
            const processedKiosks = (kioskData.devices || []).map((k: KioskDevice) => ({
                ...k,
                is_online: k.last_heartbeat ? new Date(k.last_heartbeat).getTime() > fiveMinsAgo : false
            }));
    ...
    const handleApprove = async (serial: string, adminIds: string[] | null) => {
        setActionLoading(serial);
        const res = await approveKiosk(serial, adminIds);
        if (res.error) alert(`Error: ${res.error}`);
        else setKiosks(prev => prev.map(k => k.device_serial === serial ? { ...k, status: 'approved', assigned_admin_ids: adminIds } : k));
    ...
    const handleAdminAssign = async (serial: string, adminIds: string[]) => {
        setActionLoading(serial);
        const res = await assignKioskToAdmin(serial, adminIds);
        if (res.error) {
            alert(`Error: ${res.error}`);
            throw new Error(res.error);
        } else {
            setKiosks(prev => prev.map(k => k.device_serial === serial ? { ...k, assigned_admin_ids: adminIds } : k));
        }
        setActionLoading(null);
    };
    ...
    const handleDelete = async (serial: string) => {
        setConfirmConfig({
            isOpen: true,
            title: "Delete Kiosk",
            message: `Permanently delete device ${serial}? This cannot be undone.`,
            variant: "danger",
            onConfirm: async () => {
                setConfirmConfig(prev => ({ ...prev, isOpen: false }));
                setActionLoading(serial);
    ...
    return (
        <div className="flex-1 overflow-y-auto space-y-6 animate-in fade-in duration-500 pb-8">
            {/* Stats Bar */}
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 relative z-10">
                <div className="bg-white dark:bg-gray-800 rounded-xl p-4 border border-gray-200 dark:border-gray-700 flex items-start gap-3 transform transition-all duration-300 hover:-translate-y-1 hover:shadow-md dark:hover:shadow-[0_4px_20px_rgb(255,255,255,0.05)]">
                    ...
                    {kiosks.map(kiosk => {
                        ...
                        return (
                            <div
                                ...
                                className={`bg-white dark:bg-gray-800 rounded-xl border p-5 transform transition-all duration-300 hover:-translate-y-1 hover:shadow-md dark:hover:shadow-[0_4px_20px_rgb(255,255,255,0.05)]
                            ...
                            >
                                <div className="flex flex-col lg:flex-row lg:items-center gap-4">
                                    ...
                                    {isSuperAdmin && (
                                        <div className="lg:w-1/6">
                                            <div className="flex items-center gap-1 text-[10px] text-gray-400 uppercase font-bold mb-1">
                                                <Users className="h-3 w-3" /> Assigned Admins
                                            </div>
                                            <DeviceInstructorSelector
                                                entityId={kiosk.device_serial}
                                                assignedIds={kiosk.assigned_admin_ids}
                                                instructors={systemAdmins.map(a => ({ id: a.auth_user_id, name: a.name, department_id: null }))}
                                                onSave={handleAdminAssign}
                                                isSuperAdmin={true}
                                            />
                                        </div>
                                    )}
                                    ...
                                    <div className="lg:w-1/4">
                                        ...
                                        <div className="flex items-center gap-2">
                                            ...
                                            {isSuperAdmin && (pendingRoomBindings[kiosk.device_serial] !== undefined && pendingRoomBindings[kiosk.device_serial] !== (kiosk.room_id ?? "")) ? (
                                                <button
                                    ...
                                    <div className="lg:w-1/4">
                                        ...
                                        <div className="flex items-center gap-2">
                                            <div className="relative w-full">
                                                <input
                                                    type={showPins[kiosk.device_serial] ? "text" : "password"}
                                                    maxLength={4}
                                                    defaultValue={kiosk.admin_pin || "1234"}
                                                    disabled={!isSuperAdmin}
                                                    placeholder="••••"
                                                    id={`pin-${kiosk.device_serial}`}
                                                    className="w-full text-xs pl-2 pr-8 py-1.5 rounded-lg border border-gray-200 dark:border-gray-600 bg-transparent focus:border-nwu-red outline-none transition text-gray-900 dark:text-white text-center tracking-widest font-mono disabled:opacity-50"
                                                />
                                                <button
                                                    type="button"
                                                    onClick={() => setShowPins(prev => ({ ...prev, [kiosk.device_serial]: !prev[kiosk.device_serial] }))}
                                                    className="absolute right-2 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-600 dark:hover:text-gray-300"
                                                >
                                                    {showPins[kiosk.device_serial] ? <EyeOff className="h-3.5 w-3.5" /> : <Eye className="h-3.5 w-3.5" />}
                                                </button>
                                            </div>
                                            {isSuperAdmin && (
                                                <button
                                                    onClick={() => {
                                                        ...
                                                        if (el && el.value !== (kiosk.admin_pin || "1234")) {
                                                            setConfirmConfig({
                                                                ...
                                                                message: "Are you sure you want to change the Admin PIN for this kiosk?",
                                                                variant: "warning",
                                                                onConfirm: async () => {
                                                                    setConfirmConfig(prev => ({ ...prev, isOpen: false }));
                                                    ...
                                                    className="p-1.5 rounded bg-green-50 text-green-600 hover:bg-green-100 dark:bg-green-900/20 dark:text-green-400 border border-green-200 dark:border-green-800 transition-colors shrink-0"
                                    ...
                                    {isSuperAdmin && (
                                        <div className="lg:w-1/6 flex items-center gap-2 lg:justify-end">
                                            {isLoading ? (
                                                ...
                                                <>
                                                    <button
                                                        onClick={() => handleApprove(kiosk.device_serial, kiosk.assigned_admin_ids)}
