...
export default function AdminManagementPage() {
    ...
    const toggleLock = async (admin: AdminProfile) => {
        ...
        const action = admin.is_locked ? "reactivate" : "deactivate";
        setConfirmConfig({
            isOpen: true,
            title: `${admin.is_locked ? "Reactivate" : "Deactivate"} Account`,
            message: `Are you sure you want to ${action} ${admin.name}'s account? ${!admin.is_locked ? "They will be unable to log in." : "They will be able to log in again."}`,
            variant: admin.is_locked ? "warning" : "danger",
            onConfirm: async () => {
        ...
        <div className="animate-in fade-in duration-500 font-sans">
                                                ...
                                                {!admin.is_super_admin && admin.auth_user_id !== currentUserId ? (
                                                    <select
                                                        ...
                                                        onChange={(e) => {
                                                            ...
                                                            // Reset select visually until confirmed (optional, might flicker)
                                                            // actually, since we don't update state immediately, it will uncontrolled-ly switch if we don't force it back or prevent default.
                                                            // But select value is controlled by `value` prop derived from `admin` object.
                                        ...
                                        <td className="px-6 py-4 text-right">
                                            <div className="flex items-center justify-end space-x-2">
                                                ...
                                                <button
                                                    ...
                                                    title={admin.is_super_admin ? "Administrator Protected" : admin.is_locked ? "Reactivate Account" : "Deactivate Account"}
                                                >
            ...
            {confirmModal && (
                <ConfirmationModal
                    ...
                    message={`Are you sure you want to move ${confirmModal.adminName} from ${confirmModal.currentDeptName} to ${confirmModal.targetDeptName}? This will immediately change what data they can access.`}
                    confirmLabel="Move Administrator"
                    onConfirm={async () => {
                        try {
                            await updateAdminDepartment(confirmModal.adminId, confirmModal.newDeptId || null);
                            fetchAdmins();
