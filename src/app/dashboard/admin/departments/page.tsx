...
export default async function DepartmentsPage() {
    ...
    async function addDepartment(formData: FormData) {
        ...
        const college = formData.get("college") as string || "CEAT";
        const supabase = createClient();

        // Get current user to set as owner
        const { data: { user } } = await supabase.auth.getUser();

        await supabase.from("departments").insert({ name, code, college, owner_id: user?.id });
    ...
    return (
        <div className="space-y-6">
            ...
            {isSuperAdmin && (
                <div className="bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 transform transition-all duration-300 hover:-translate-y-1 hover:shadow-md dark:hover:shadow-[0_4px_20px_rgb(255,255,255,0.05)]">
            ...
            {/* Departments List */}
            <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 overflow-hidden transform transition-all duration-300 hover:-translate-y-1 hover:shadow-md dark:hover:shadow-[0_4px_20px_rgb(255,255,255,0.05)]">
                        ...
                        <tbody className="divide-y divide-gray-100 dark:divide-gray-700">
                            {departments?.map((dept) => (
                                <tr key={dept.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors group">
                                    <td className="px-6 py-4 font-bold text-gray-500 dark:text-gray-400 text-xs">{dept.college || 'CEAT'}</td>
