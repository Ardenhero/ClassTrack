...
export default function RoomEnvironment() {
    ...
    const handleToggle = async (device: IoTDevice) => {
        ...
        try {
            ...
            // 1. Update the device state in Supabase
            // This triggers the ESP32 local bridge via Realtime.
            ...
            // Optimistically update local state (if Realtime is slow)
            setDevices((prev) =>
                prev.map((d) =>
            ...
            // Revert state if needed? Actually Realtime will bring back the true DB state anyway.
        } finally {
            // Give it 3s for the ESP32 to respond and Realtime to sync back
            setTimeout(() => setToggling((prev) => (prev === device.id ? null : prev)), 3000);
        }
    };
